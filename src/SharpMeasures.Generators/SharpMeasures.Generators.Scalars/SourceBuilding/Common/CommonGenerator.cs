...
internal static class CommonGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<Scalars.DataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Scalar.Type, model.Value.Scalar.Unit, unit.Definition.Quantity, unitParameterName, model.Value.Scalar.UseUnitBias, model.Value.Scalar.DefaultUnitInstanceName, model.Value.Scalar.DefaultUnitInstanceSymbol, model.Value.SourceBuildingContext);
    }
}
