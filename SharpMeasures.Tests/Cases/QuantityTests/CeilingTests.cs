...
namespace ErikWe.SharpMeasures.Tests.Cases.QuantityTests
{
    public class CeilingTests
    {
        ...
        {
            Scalar result = a.Ceiling();

            Assert.Equal(Math.Ceiling(a.Magnitude), result.Magnitude, 2);
        }
        ...
        {
            IScalarQuantity result = a.Ceiling();

            Assert.Equal(a.Magnitude.Ceiling(), result.Magnitude, 2);
        }
    }
}
