...
");
    ...
    public static void AppendUsings(StringBuilder source, ISymbol symbol, IEnumerable<string> usingsNames)
    {
        ...
        IEnumerable<string> toLines()
        {
            ...
            {
                if (!implicitUsing.StartsWith(usingName))
                {
        ...
        bool initialNewLine = true, bool finalNewLine = true)
    {
        ...
        AppendClosingBracket(source, indentLevel, initialNewLine: !source.ToString().EndsWith(Environment.NewLine), finalNewLine);
    }
    ...
    public static string ToParameterName(string name) => name.Substring(0, 1).ToLower() + name.Substring(1);
}
