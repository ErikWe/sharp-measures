...
internal class DefaultIndividualVectorDocumentation : IIndividualVectorDocumentationStrategy, IEquatable<DefaultIndividualVectorDocumentation>
{
    ...
    public DefaultIndividualVectorDocumentation(IndividualVectorDataModel model)
    {
        Type = model.Vector.Type;
        Dimension = model.Vector.Definition.Dimension;
        Unit = model.Vector.Definition.Unit;

        DefaultUnit = model.Vector.Definition.DefaultUnit;
        DefaultUnitSymbol = model.Vector.Definition.DefaultUnitSymbol;

        UnitParameterName = SourceBuildingUtility.ToParameterName(Unit.Type.Name);

        if (model.Vector.Definition.Scalar is not null && model.ScalarPopulation.Scalars.TryGetValue(model.Vector.Definition.Scalar.Type.AsNamedType(), out var scalar))
        {
            ExampleScalarBase = scalar.IncludedBases.Count > 0 ? scalar.IncludedBases[0] : null;
        }

        ExampleUnit = model.Vector.IncludedUnits.Count > 0 ? model.Vector.IncludedUnits[0] : null;
    ...
    public string Constant(IVectorConstant constant)
    {
        ...
        return $$"""/// <summary>The {{ScalarReference}} representing the constant {{constant.Name}}, with value { ({{componentText}}) [{{constant.Unit.Plural}}] }.</summary>""";
    ...
    public static bool operator ==(DefaultIndividualVectorDocumentation? lhs, DefaultIndividualVectorDocumentation? rhs) => lhs?.Equals(rhs) ?? rhs is null;
    public static bool operator !=(DefaultIndividualVectorDocumentation? lhs, DefaultIndividualVectorDocumentation? rhs) => (lhs == rhs) is false;
