...
{
    public abstract (IncrementalValueProvider<IScalarPopulation>, IScalarGenerator) Resolve(IncrementalGeneratorInitializationContext context,
...
internal class ScalarResolver : IScalarResolver
{
    ...
    internal ScalarResolver(IncrementalValuesProvider<UnresolvedScalarBaseType> scalarBaseProvider,
        IncrementalValuesProvider<UnresolvedScalarSpecializationType> scalarSpecializationProvider)
    {
        ScalarBaseProvider = scalarBaseProvider;
        ScalarSpecializationProvider = scalarSpecializationProvider;
    }

    public (IncrementalValueProvider<IScalarPopulation>, IScalarGenerator) Resolve(IncrementalGeneratorInitializationContext context,
        IncrementalValueProvider<IUnresolvedUnitPopulation> unitPopulationProvider, IncrementalValueProvider<IUnresolvedScalarPopulation> scalarPopulationProvider,
        IncrementalValueProvider<IUnresolvedVectorPopulation> vectorPopulationProvider)
    {
        var resolvedScalarBases = ScalarBaseProvider.Combine(unitPopulationProvider, scalarPopulationProvider, vectorPopulationProvider)
            .Select(ScalarBaseTypeResolution.Resolve).ReportDiagnostics(context);
        ...
        var reducedScalarSpecializations = resolvedScalarSpecializations.Combine(intermediatePopulation).Select(ScalarSpecializationTypeResolution.Reduce)
    ...
    private static IIntermediateScalarPopulation CreateIntermediatePopulation
        ((ImmutableArray<IScalarType> Bases, ImmutableArray<IIntermediateScalarSpecializationType> Specializations) scalars, CancellationToken _)
    {
        return new IntermediateScalarPopulation(scalars.Bases.ToDictionary(static (scalar) => scalar.Type.AsNamedType()),
            scalars.Specializations.ToDictionary(static (scalar) => scalar.Type.AsNamedType()));
    }
