...
internal static class DerivationsGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<Scalars.DataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Scalar.Type, model.Value.Scalar.DefinedDerivations.Concat(model.Value.Scalar.InheritedDerivations).ToList(), model.Value.ScalarPopulation, model.Value.Documentation);
    }
}
