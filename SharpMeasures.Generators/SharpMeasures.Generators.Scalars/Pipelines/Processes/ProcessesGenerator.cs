...
internal static class ProcessesGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<Scalars.DataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Scalar.Type, model.Value.Scalar.Processes.Concat(model.Value.Scalar.InheritedProcesses).ToList(), model.Value.Documentation);
    }
}
