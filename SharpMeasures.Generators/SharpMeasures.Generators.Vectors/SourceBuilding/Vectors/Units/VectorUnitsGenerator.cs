...
internal static class VectorUnitsGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<VectorDataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Vector.Type, model.Value.Vector.Dimension, model.Value.Vector.Scalar, model.Value.Vector.Unit, unit.Definition.Quantity, includedUnitInstances, model.Value.Vector.Constants.Concat(model.Value.Vector.InheritedConstants).ToList(), model.Value.SourceBuildingContext);
    }
