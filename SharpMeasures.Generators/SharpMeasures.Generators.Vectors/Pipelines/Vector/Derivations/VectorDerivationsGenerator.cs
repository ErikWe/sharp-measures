...
internal static class VectorDerivationsGenerator
{
    ...
    {
        if (model.HasValue is false || model.Value.Vector.Derivations.Count is 0 && model.Value.Vector.InheritedDerivations.Count is 0)
        {
            return new Optional<DataModel>();
        }

        return new DataModel(model.Value.Vector.Type, model.Value.Vector.Dimension, model.Value.Vector.Scalar is not null, model.Value.Vector.Derivations.Concat(model.Value.Vector.InheritedDerivations).ToList(), model.Value.ScalarPopulation, model.Value.Documentation);
    }
}
