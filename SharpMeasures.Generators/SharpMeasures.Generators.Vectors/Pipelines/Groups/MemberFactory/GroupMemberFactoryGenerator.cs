...
internal static class GroupMemberFactoryGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<GroupDataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Group.Type, model.Value.Group.Unit, unitParameterName, model.Value.Group.Scalar, model.Value.Group.MembersByDimension, model.Value.Documentation);
    }
}
