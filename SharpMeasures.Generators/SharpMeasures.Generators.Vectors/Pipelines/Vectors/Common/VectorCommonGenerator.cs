...
internal static class VectorCommonGenerator
{
    ...
    private static Optional<DataModel> Reduce(Optional<VectorDataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Vector.Type, model.Value.Vector.Dimension, model.Value.Vector.Unit, unit.Definition.Quantity, unitParameterName, model.Value.Vector.Scalar, model.Value.Vector.DefaultUnitInstanceName, model.Value.Vector.DefaultUnitInstanceSymbol, model.Value.Documentation);
    }
}
