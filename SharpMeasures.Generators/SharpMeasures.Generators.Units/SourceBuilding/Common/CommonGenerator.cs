...
internal static class CommonGenerator
{
    ...
    private static Optional<DataModel> ReduceToDataModel(Optional<Units.DataModel> model, CancellationToken _)
    {
        ...
        return new DataModel(model.Value.Unit.Type, model.Value.Unit.Definition.Quantity, model.Value.Unit.Definition.BiasTerm, SourceBuildingUtility.ToParameterName(model.Value.Unit.Definition.Quantity.Name), model.Value.SourceBuildingContext);
    }
}
