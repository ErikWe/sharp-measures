...
internal static class Execution
{
    ...
    private sealed class Composer
    {
        ...
        private void AppendFixedUnitInstances(Indentation indentation)
        {
            ...
            AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.FixedUnitInstance(Data.FixedUnitInstance));
        ...
        private void AppendDerivedUnitInstances(Indentation indentation)
        {
            foreach (var derivedUnit in Data.DerivedUnitInstances)
            {
                ...
                AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.DerivedUnitInstance(derivedUnit));
        ...
        private void AppendDependantUnits(Indentation indentation, IList<IModifiedUnitInstance> dependantUnits)
        {
            ...
            for (int i = 0; i < dependantUnits.Count; i++)
            {
                if (ImplementedDefinitions.Contains(dependantUnits[i].OriginalUnitInstance))
                {
                    ...
                    {
                        AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.UnitAliasInstance(unitAlias));
                        commonDelegate();
                        AppendAlias(unitAlias);
                    }
                    else if (dependantUnits[i] is IScaledUnitInstance scaledUnit)
                    {
                        AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.ScaledUnitInstance(scaledUnit));
                        commonDelegate();
                        AppendScaled(scaledUnit);
                    }
                    else if (dependantUnits[i] is IPrefixedUnitInstance prefixedUnit)
                    {
                        AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.PrefixedUnitInstance(prefixedUnit));
                        commonDelegate();
                        AppendPrefixed(prefixedUnit);
                    }
                    else if (dependantUnits[i] is IBiasedUnitInstance biasedUnit)
                    {
                        AppendDocumentation(indentation, Data.SourceBuildingContext.Documentation.BiasedUnitInstance(biasedUnit));
                        commonDelegate();
