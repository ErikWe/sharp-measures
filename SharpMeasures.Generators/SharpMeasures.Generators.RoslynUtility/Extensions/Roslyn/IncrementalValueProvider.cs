...
{
    public static IncrementalValueProvider<T> ExtractResult<T>(this IncrementalValueProvider<IResultWithDiagnostics<T>> provider) => provider.Select(static (result, _) => result.Result);

    public static IncrementalValueProvider<T> ReportDiagnostics<T>(this IncrementalValueProvider<IResultWithDiagnostics<T>> provider, IncrementalGeneratorInitializationContext context)
    {
    ...
    public static IncrementalValueProvider<ImmutableArray<T>> Concat<T>(this IncrementalValueProvider<ImmutableArray<T>> provider1, IncrementalValueProvider<ImmutableArray<T>> provider2)
    {
        ...
        static ImmutableArray<T> concatArrays((ImmutableArray<T> Left, ImmutableArray<T> Right) arrays, CancellationToken _) => arrays.Left.AddRange(arrays.Right);
    }
