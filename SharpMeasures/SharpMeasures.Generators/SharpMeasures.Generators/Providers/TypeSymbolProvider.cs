...
internal static class TypeSymbolProvider
{
        ...
        DOutputTransform<TIn, TOut> outputTransform)
    {
        ...
        TOut? extractSymbol(TIn input, CancellationToken _)
        {
            if (ExtractSymbol(inputTransform(input)) is not INamedTypeSymbol typeSymbol)
            {
                return default;
            }

            return outputTransform(input, typeSymbol);
        }
    }
