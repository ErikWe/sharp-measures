...
/// This is the magnitude of the vector quantity <see cref="Yank3"/>, and is expressed in <see cref="UnitOfYank"/>, with the SI unit being [kg].
/// <para>
...
/// </item>
/// </list>
/// The magnitude of the measure can be retrieved using pre-defined properties, prefixed with 'In', followed by the desired <see cref="UnitOfYank"/>.
/// </para>
/// </summary>
    ...
    IVector3MultiplicableScalarQuantity<Yank3, Vector3>
{
    ...
    /// <summary>Constructs a new <see cref="Yank"/>, with magnitude <paramref name="magnitude"/> in <see cref="UnitOfYank"/> <paramref name="unitOfYank"/>.</summary>
    /// <param name="magnitude">The magnitude of the <see cref="Yank"/>, in <see cref="UnitOfYank"/> <paramref name="unitOfYank"/>.</param>
    ...
    /// <remarks>Consider preffering a constructor that requires a <see cref="UnitOfYank"/> to be specified.</remarks>
    public Yank(Scalar magnitude) : this(magnitude.Magnitude) { }
    /// <summary>Constructs a new <see cref="Yank"/>, with magnitude <paramref name="magnitude"/>.</summary>
    /// <param name="magnitude">The magnitude of the <see cref="Yank"/>.</param>
    /// <remarks>Consider preferring a constructor that requires a <see cref="UnitOfYank"/> to be specified.</remarks>
    ...
    /// <summary>Produces a <see cref="Yank"/>, with magnitude equal to the absolute of the original magnitude.</summary>
    public Yank Absolute() => new(Math.Abs(Magnitude));
    /// <summary>Produces a <see cref="Yank"/>, with magnitude equal to the floor of the original magnitude.</summary>
    public Yank Floor() => new(Math.Floor(Magnitude));
    /// <summary>Produces a <see cref="Yank"/>, with magnitude equal to the ceiling of the original magnitude.</summary>
    public Yank Ceiling() => new(Math.Ceiling(Magnitude));
    /// <summary>Produces a <see cref="Yank"/>, with magnitude equal to the original magnitude, rounded to the nearest integer.</summary>
    ...
    /// <summary>Produces a <see cref="Scalar"/> with magnitude equal to that of the <see cref="Yank"/>, expressed in <see cref="UnitOfYank"/>
    /// <paramref name="unitOfYank"/>.</summary>
    ...
    /// <summary>Produces a <see cref="Scalar"/> from the magnitude of a <see cref="Yank"/>, expressed in <see cref="UnitOfYank"/>
    /// <paramref name="unitOfYank"/>.</summary>
    /// <param name="yank">The <see cref="Yank"/> to be expressed in <see cref="UnitOfYank"/> <paramref name="unitOfYank"/>.</param>
    /// <param name="unitOfYank">The <see cref="UnitOfYank"/> in which the magnitude is expressed.</param>
    private static Scalar InUnit(Yank yank, UnitOfYank unitOfYank) => new(yank.Magnitude / unitOfYank.Factor);
    ...
    /// <summary>Multiplies the <see cref="Unhandled"/> quantity <paramref name="y"/> by the <see cref="Yank"/> <paramref name="x"/> -
    /// resulting in an <see cref="Unhandled"/> quantity.</summary>
    ...
    /// <param name="y">The remainder is retrieved from division of <see cref="Yank"/> <paramref name="x"/> by this value.</param>
    ...
    /// by <paramref name="y"/>.</summary>
    /// <param name="x">The <see cref="Yank"/>, which is divided by <paramref name="y"/> to produce a remainder.</param>
    /// <param name="y">The remainder is retrieved from division of the <see cref="Yank"/> <paramref name="x"/> by this value.</param>
    ...
    /// <summary>Multiplies the <see cref="Yank"/> with the <see cref="ValueTuple"/> <paramref name="components"/> to produce a <see cref="Yank3"/>.</summary>
    /// <param name="components">This <see cref="ValueTuple"/> is multiplied by the <see cref="Yank"/>.</param>
    ...
    /// <summary>Multiplies the <see cref="Yank"/> <parmref name="b"/> with the <see cref="ValueTuple"/> <paramref name="a"/> to produce a <see cref="Yank3"/>.</summary>
    ...
    /// <summary>Converts the <see cref="Yank"/> to a <see cref="double"/> with value <see cref="Magnitude"/>.</summary>
    public double ToDouble() => Magnitude;
    /// <summary>Converts the <see cref="Yank"/> to a <see cref="double"/> based on the magnitude of the <see cref="Yank"/> <paramref name="x"/>.</summary>
    public static implicit operator double(Yank x) => x.ToDouble();

    /// <summary>Converts the <see cref="Yank"/> to the <see cref="Scalar"/> of equivalent magnitude.</summary>
