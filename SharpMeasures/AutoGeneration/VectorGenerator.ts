...
export class VectorGenerator {
    ...
    private async generateVector(vector: VectorQuantity, dimensionality: number): Promise<void> {
        ...
        const interfacesText: string = this.composeInterfacesText()
    ...
    private insertNames(text: string, vector: VectorQuantity, dimensionality: number): string {
        ...
        text = text.replace(new RegExp('#UnitVariable#', 'g'), lowerCase(vector.unit))
        ...
        text = text.replace(new RegExp('#quantity#', 'g'), lowerCase(vector.name + dimensionality))
    ...
    private setComponentListTexts(vector: VectorQuantity, dimensionality: number, text: string): string {
        const componentLists: { replace: string, append: (name: string) => string, slice: (text: string) => string }[] = [{
                ...
                append: (name: string) =>
                    '\t#Document:Component' + name + '(#Quantity#, #Dimensionality#)#\n' +
                    '\tpublic double ' + name + ' { get; init; }\n',
                slice: (text: string) => text.slice(0, -1)
            }, {
                ...
                slice: (text: string) => text.slice(0, -2)
            }, {
                ...
                append: (name: string) => name + name + ' * factor.Magnitude, ',
                slice: (text: string) => text.slice(0, -2)
            }, {
    ...
    private composeInterfacesText(): string {
