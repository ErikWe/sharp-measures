...
The goal is for SharpMeasures to include the most common quantites, and to be sufficient for most cases. However, you might prefer your method signature to consist of the more explicit `Altitude` rather than an ambiguous `Length`. SharpMeasures will not attempt to include all such specialized quantities, but they can easily be generated by the SharpMeasures Source Generator:
